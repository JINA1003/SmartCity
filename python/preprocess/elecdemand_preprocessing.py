...
# ------------수요 감축 필요도 계산-------------
# 공급예비율 데이터 불러오기
rate = pd.read_csv('data/output/epsis_supply_rate_final_20052026.csv')
...
idx_df = idx_df.rename(columns={
    ...
    '월': 'month',
    '전력사용량(MWh)': 'power_mwh',
    '연도_월별_시군구별_전력사용량합계': 'total_power_mwh',
    '전력사용율': 'usage_ratio',
    '공급능력(MW)': 'capacity_mw',
    '최대전력(MW)': 'peak_mw',
    '공급예비력(MW)': 'reserve_mw',
    ...
    '공급위험도': 'risk_score',
    '용도정규화사용률': 'norm_usage',
    '수요감축필요도_1st': 'reduction_need'
})

print('수요 감축 필요도 계산 완료')

idx_df.to_csv("data/output/kepco_final_20052026.csv", index=False, encoding="utf-8-sig")

print("최종 데이터 저장 완료")
print("저장위치: 'data/output/kepco_final_20052026.csv'")
